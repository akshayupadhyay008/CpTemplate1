...
ill solve(ill i,ill arr[],ill num ,ill k){
    if (i>num-1) {
        return 0;
    }
    if (i==num-1) {
    ...
    // if (i==num-2) {
    //     return dp[i]=abs(arr[i]-arr[i+1])+solve(i+1,arr,num);
    // }
    // else {
    //     return dp[i]=min(abs(arr[i]-arr[i+1])+solve(i+1,arr,num),abs(arr[i]-arr[i+2])+solve(i+2,arr,num));
    // }

    ill mx=inf;
    f1(j,1,k) {
        if (i+j<=num-1) {
            mx=min(mx,abs(arr[i]-arr[i+j]) + solve(i+j,arr,num,k));
        }
    }
