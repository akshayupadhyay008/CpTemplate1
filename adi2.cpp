...
int main(){    
    ...
    */
    ...
    // initialising the dependency as indistinguishable/same at present as 0 
    // -1 will means distinguishable/different 
    // value[x][y]=0 means state x and y are same/indistinguishable
    // value[x][y]=-1 means state x and y are different/distinguishable
    ...
    */
    ...
    // if at the end we find that the two states are not different so that will imply that they are same 
    for(int i=0;i<num-1;i++) {
        for(int j=i+1;j<num;j++) {
            int state1=state[i];
            int state2=state[j];
