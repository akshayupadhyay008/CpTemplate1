...
#define newint(...) int _VA_ARGS;read(VA_ARGS_);
...
pair<int,int> dfs(int x, int y, int n, int m, vector<string> &v, vector<vector<bool>> &vis)
{
    ...
    if(x > 0) {
        if(v[x-1][y] == v[x][y] and vis[x-1][y] == false) {
            auto z = dfs(x-1,y,n,m,v,vis);
            res.first += z.first;
            res.second += z.second;
        }
    }
    ...
    if(y > 0) {
        if(v[x][y-1] == v[x][y] and vis[x][y-1] == false) {
            auto z = dfs(x,y-1,n,m,v,vis);
            res.first += z.first;
            res.second += z.second;
        }
    }
...
void solve()
{
    ...
    for(int i=0;i<n;i++) {
        for(int j=0;j<m;j++) {
            if(!vis[i][j]) {
                auto x = dfs(i,j,n,m,v,vis);
                ans += min(x.first, x.second);
            }
        }
    } 
    cout << ans << endl;
