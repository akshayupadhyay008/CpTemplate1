...
int ok(ill arr[],ill mid,ill k,ill num) {
    // if (x<=key) return 1;
    // else return 0;

    ill inc=0;
    f0(i,num/2,num){
        inc=inc+max(0LL,mid-arr[i]);
    }
...
ill solve(ill arr[],ill i,ill cnt1, ill cnt2, ill num) {
    ...
    if (arr[i]==0) {
        return solve(arr,i+1,cnt1,cnt2,num);
    }
    
    if (cnt1==0) {
        return cnt2*solve(arr,i+1,cnt1,cnt2-1,num);
    }
    else if (cnt2==0) {
        return cnt1*solve(arr,i+1,cnt1-1,cnt2,num);
    }
    else {
        return cnt2*solve(arr,i+1,cnt1,cnt2-1,num) + cnt1*solve(arr,i+1,cnt1-1,cnt2,num);
    }
...
int main() {
    ...
    f1(ic,1,t) {
        ...
        ill ans=solve(arr,1,cnt1,cnt2,num);
