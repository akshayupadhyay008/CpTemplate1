...
// this is similar to a set 
// not a multiset 
// in case of multiset --> instead of bool end we will have int end
// in case of insert we will do end++
// in case of delete we will erase end--
struct node {
    // adjacency list 
    // color of end 
    node* adj[26];
    bool end;
    int cnt; // number of strings which are going down from here // which will help in finding the number of strings with prefix as this 
};
...
void insert(string& str) {
    ...
    for(char ch : str) {
        int d=ch-'a'; // a->0  b->1  c->2  d->3 ..
        if (cur->adj[d] == NULL) {
            cur->adj[d] = create_node();
        }
        cur = cur->adj[d];
        cur->cnt++;
    }

    // we have reached the end --> so we will mark it true 
    cur->end=true;
}
...
int strings_with_this_prefix(string& str) {
    node* cur = root;
