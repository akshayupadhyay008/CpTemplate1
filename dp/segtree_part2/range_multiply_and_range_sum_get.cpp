...
struct segtree {
    ...
    vector <ill> add; // stores the minimum of the node --> with all opertions in and on below of this node(till leaf) already applied 
    ...
    void init(ill n) {
        ...
        tree.assign(2*size+1,1LL);
        add.assign(2*size+1,0LL);
    }
    
    void build(ill num,ill x,ill lx,ill rx) {
        // cout << num << " "  << x << " " << lx <<  "  " << rx << endl;
        if (lx==rx) {
            if (lx<=num) {
                add[x] = 1;
                // cout << 1 << " " << add[x] << endl;
        ...
        ill mid=(rx+lx)/2;
        build(num,2*x,lx,mid);
        build(num,2*x+1,mid+1,rx);
        add[x] = (add[2*x] + add[2*x+1])%mod;
        // cout << 2 << "  " << add[x] << endl;
    }
    ...
    ill range_get(ill l,ill r,ill x,ill lx,ill rx) { // index is used to direct which side to get 
        ...
        ill mid=(lx+rx)/2;
        ill res1 = range_get(l,r,2*x,lx,mid)%mod;
        ill res2 = range_get(l,r,2*x+1,mid+1,rx)%mod;

        ill res = (res1 + res2)%mod;
        //  + tree[x]; // as the operation stored in this node is not applied to the res1 and res2 so we apply it now 
        // ill left = max(l,lx);
        // ill right = min(r,rx);
        // res = (res + (tree[x] * (right-left+1))%mod)%mod;
        res = (res%mod * tree[x]%mod)%mod;
        return res%mod;
    }
    ...
    void range_update(ill l,ill r,ill val,ill x,ill lx,ill rx) { // add val to the range [l,r]
        ...
        if (lx >= l && rx <= r) { // complete intersection --> add it here and return 
            // tree[x] = (tree[x] + val)%mod; // #merge 2 operation
            tree[x] = (tree[x]%mod * val%mod)%mod; // #merge 2 operation
            // we also want to recalculate the addition  
            // add[x] = (add[x] + ((rx-lx+1)*val)%mod)%mod; // if all the values in this node is increased by val then add is  increased by val * length 
            add[x] = (add[x]%mod * val%mod)%mod;
            return;
        }
        ...
        add[x] = (add[2*x]%mod + add[2*x+1]%mod)%mod;
        // we apply the operation here also 
        // add[x] = (add[x] + ((rx-lx+1) * tree[x])%mod)%mod;
        add[x] = (add[x]%mod * tree[x]%mod)%mod;
    }
...
int main() {
    ...
    f1(ic,1,tt) {
        ...
        // 0*1=0
        // f1(i,1,num) {
        //     s.range_update(i,i,1);
        // }
        // initialise sum as 1 if you want all values to be 1 --> nooo

        // use build to build the sum 
        s.build(num);
