...
struct segtree {
    ...
    ill range_get(ill l,ill r,ill x,ill lx,ill rx) { // index is used to direct which side to get 
        ...
        ill res = (res1 + res2);
        // intersection of both the segments 
        ill left = max(l,lx);
        ill right = min(r,rx);
        res = res + (tree[x] * (right-left+1));
        return res;
    }
    ...
    void range_update(ill l,ill r,ill val,ill x,ill lx,ill rx) { // add val to the range [l,r]
        ...
        if (lx >= l && rx <= r) { // complete intersection --> add it here and return 
            ...
            add[x] = add[x] + ((rx-lx+1)*val); // if all the values in this node is increased by val then add is  increased by val * length 
            return;
        }
        ...
        add[x] = (add[2*x] + add[2*x+1]); 
        add[x] = add[x] + ((rx-lx+1) * tree[x]);
