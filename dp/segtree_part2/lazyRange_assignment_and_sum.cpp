...
struct segtree {
    ...
    ill no_opertation; // will tell that we have no operation here 
    ...
    void propagate(ill x,ill lx,ill rx) {  // but change the mini value also 
        ...
        // if (oldOp == no_opertation) {
        //     tree[2*x] = oldOp;
        //     mini[2*x] = mini[2*X]; // do not change 
        // }
        // // else combine 
    ...
    ill range_get(ill l,ill r,ill x,ill lx,ill rx) { // index is used to direct which side to get 
        ...
        return res1 + res2; // as the operation stored in this node is not applied to the res1 and res2 so we apply it now 
    }
    ...
    void range_update(ill l,ill r,ill val,ill x,ill lx,ill rx) { // add val to the range [l,r]
        ...
        if (lx >= l && rx <= r) { // complete intersection --> add it here and return 
            tree[x] = val; // #merge 2 operation   --> operation change / propagate 
            // we also want to recalculate the minimums 
            mini[x] = (rx-lx+1) * val; // if all the values in this node is increased by val then minimum is also increased by val // value propagate / change 
            return;
        }
