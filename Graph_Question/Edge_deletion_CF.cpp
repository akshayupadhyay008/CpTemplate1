...
int main() {
    ...
    f1(ic,1,tt) {
        ...
        vector <ill> ans;
        vector <ill> dis(num+1,inf);

        set <pii> s;

        s.insert({0,1});
        dis[1]=0;
        vector <ill> par(num+1,-1);
        ...
        while(1) {
            ...
            if (par[root]!=-1) ans.pb((edge[{root,par[root]}]));
            ...
            for(auto x: adj[root]) {
                if (dis[x.fi] >= d+x.se) {
                    if (s.find({dis[x.fi],x.fi}) != s.end()) {
                        s.erase({dis[x.fi],x.fi});
                    }
                    dis[x.fi]=d+x.se;
                    par[x.fi]=root;
                    s.insert({d+x.se,x.fi});
                }
            }
        }
