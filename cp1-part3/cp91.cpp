...
int main() {
    ...
    f1(ic,1,tt) { 
        ...
        vector < pair < pii, ill >  > v;

        f0(i,0,num) {
            ill dis1=abs(x1-arr[i].fi) * abs(x1-arr[i].fi)  + abs(y1-arr[i].se)  * abs(y1-arr[i].se) ;
            ill dis2=abs(x2-arr[i].fi) * abs(x2-arr[i].fi)  + abs(y2-arr[i].se)  * abs(y2-arr[i].se) ;
            v.pb({{dis1,i},1});
            v.pb({{dis2,i},2});
        }   
