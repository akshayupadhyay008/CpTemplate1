...
ill diameter(ill root,ill num,vector <ill>* adj,vector <int>& vis,ill & dia){
    ill mx1=0;
    ill mx2=0;
    ...
    for(auto x:adj[root]) {
        if (!vis[x]) {
            ill val=diameter(x,num,adj,vis,dia);
            if (val>=mx1) {
                mx2=mx1;
                mx1=val;
            }
...
// also note that diameter can be found using 2 dfs or 2 bfs 
// step 1 --> call a dfs from a random node and find one of the deepest node 
// note --> Yogesh sir said --> any one of the deepest node will work to find the diameter 
// step 2 --> call dfs from that deepest node and the max depth from there is the answer 
...
int main() {
    ...
    f1(ic,1,tt) {
        ...
        f1(j,1,num){
            ...
            ill dummy=diameter(1,n,adj,vis,dia); // return your max height 
