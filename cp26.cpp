...
int checker(vector <ill> v,ill num,ill days,ill mx) {
    ill odd=(days/2) + (days%2);
    ill even=(days/2);
...
ill ok (ill arr[],ill num,ill mx) {
    ...
    ill ans=inf;
    f0(i,0,100) {
        int c1=checker(v,num,(sum/3)+i,mx);
        if (c1==1) ans=min(ans,sum/3+i);

        if (sum/3-i >= 0) {
            int c2=checker(v,num,sum/3-i,mx);
            ans=min(ans,sum/3-i);
        }
...
int main() {
    ...
    f1(ic,1,tt) {
        ...
        ill ans1=ok(arr,num,mx);
        ill ans2=ok(arr,num,mx+1);
        ill ans3=ok(arr,num,mx+2);

        ill ans=min(ans1,min(ans2,ans3));
        cout<<ans<<endl;
        ...
        //     if ((ok(arr,num,mid,mx) || ok(arr,num,mid,mx+1)) || ok(arr,num,mid,mx+2)){
        //         hi=mid;
        //     }
        //     else {
        //         lo=mid;
