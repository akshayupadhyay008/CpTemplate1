...
//     }    
    ...
    ill dp[num+10][sum+10]; // 1 means this subset sum is possible and 0 means that this is not possible 
    ...
    f1(i,1,sum) { // sum=1 to sum=sum
        dp[0][i]=0; // if no element then sum not possible 
    }

    f1(i,1,num) { // num = 1 to num= num // 0 sum possible 
        dp[i][0]=1;
    }
    ...
    f1(i,1,num){
        f1(j,1,sum){
            if (dp[i-1][j] == 1) {
                ...
                dp[i][j]=max(dp[i-1][j],dp[i-1][j-arr[i-1]]) ; // any one out of the two is 1 then this is also 1 so max works here 
            }
        }
    }
