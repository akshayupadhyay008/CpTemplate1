...
//     }
    ...
    // finding the msb(most significant bit) of the max number because the XOR cannot be greater than the after msb all 1 
    // say max number is   1001001  so the xor can be max to max 
    //                ==   1111111
    ...
    ill dp[num+5][maxXOR+5]; // 1 means this subset xor is possible and 0 means that this is not possible 
    ...
    dp[0][0]=1; // 0 element 0 sum possible 
    //  If P is empty, then F(P)=0.

    f1(i,1,maxXOR) { // xor value=1 to xor value=max xor  
        dp[0][i]=0; // if no element then this xor is not possible  
    }

    f1(i,1,num) { // num = 1 to num= num // 0 xor possible (take the empty)
        dp[i][0]=1;
    }

    f1(i,1,num){
        f1(j,1,maxXOR){
            if (dp[i-1][j] == 1) { // not take arr[i]
                dp[i][j]=1;
            }
            else { // take arr[i]
                ill val=j^arr[i-1];
