...
// -1 -1 -1 -1 -
        ...
        // i j   dp[k]>=i --> k can be formed 
        //         < i --> cannot be formed 
        // i se j   k  --> T/F 

        int i=0,j=0,ans=intmax;
        
        // i=0;
        // j=0;
        // ans=inf;

        // i j     dp[k] = max index (idx --> j) sum
        ...
        while(i<n and j<n)
        {
            ...
            for(int k=s;k>=a[j];k--)  // 10 9 
            {
                dp[k]=max(dp[k],dp[k-a[j]]); // 0 -1 
            }
