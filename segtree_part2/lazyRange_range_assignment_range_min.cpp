...
struct segtree {
    ...
    ill range_get(ill l,ill r,ill x,ill lx,ill rx) { // index is used to direct which side to get 
        ...
        return min(res1,res2) ; // as the operation stored in this node is not applied to the res1 and res2 so we apply it now 
    }
    ...
    void range_update(ill l,ill r,ill val,ill x,ill lx,ill rx) { // add val to the range [l,r]
        ...
        if (lx >= l && rx <= r) { // complete intersection --> add it here and return 
            ...
            mini[x] =  val; // if all the values in this node is increased by val then minimum is also increased by val // value propagate / change 
            return;
        }
