...
int main() {
    ...
    f1(ic,1,tt) {
        ...
        f0(r,0,num) { // ending point 0 se num-1 
            // we have stepped into the next element so add arr[r] in the sum
            sum=sum + arr[r]; // acquire 
            ...
            while(1) { // release  
                if (sum > x) {
                    ...
                    break; 
                    // remove from the end till we get the sum <= x  -->  [l,r]  l and r enclusive ==> sum <= x 
                    // a case(not the only case) : if my arr[i] is huge and > x so none element so l=r+1 (l will become r+1) 
                    // then if you do length of segment   r - l + 1  =    r-(r+1)+1=0 so correct EMPTY 
                    // also in the next acquire we will acquire it again l=4  r=3     next iteration   l=4 r=4 
                }
            }

            // if condition is correct then update answer 
            // sum will be <= x  here as we have already checked and came 
            // ans=max(ans,r-l+1);
            // L --> R subarray 
            // L start      R end 
            // start at L L+1 L+2 ... all are good .. start at R
            ans += (r-l+1);
        }
