...
int ok(ill arr[],string& str,ill strlen,string& t,ill tlen,ill mid) {
    vector <int> isThere(strlen,1);
    ...
    vector <int> pi=prefix_function(sss);
...
int main() {
    ...
    f1(ic,1,tt) {
        ...
        ill hi=strlen+1; // invalid candidate 
        ill mid;

        while(hi>lo+1){
            mid=lo+(hi-lo)/2;
            if (ok(arr,str,strlen,t,tlen,mid)){
                lo=mid;
            }
            else {
                hi=mid;
