...
ill solve(ill i,ill prev,ill x,ill k,ill arr[],ill num){
    if (i==num){    
        return 0;
    }

    // each has 2 choices --> take or not take 
    if (x==0) { // not take 
        if (prev==0) {
            return intmin;
        }
        else {
            ill prevmn=max(0LL,i-k+1);
            ill prevhere=prev-1;
            if (prevhere<prevmn) {
                return intmin;
            }
            else {
                return solve(i+1,prev,x,k,arr,num);
            }
        }
    }
    else {
        // 2 choices  --> 2 or 1 depends 
        ill prevmn=max(0LL,i-k+1);
        ill prevhere=prev-1;
        ...
        if (prevhere<prevmn) { 
            // return intmin;
            // 1 choice place here 
            return arr[i]+solve(i+1,i+1,x-1,k,arr,num);
        }
        else { // 2 choice 
            return max(solve(i+1,prev,x,k,arr,num),arr[i]+solve(i+1,i+1,x-1,k,arr,num));
        }
    }
}
...
int main() {
    ...
    f1(ic,1,tt) {
        ...
        ill i=0;
        ill prev=0;// --> no prev 
        // prev = 1 means --> 0 pe prev (because want to store in dp)
        ill ans=solve(0,prev,x,k,arr,num);
